...
<p>Shootings<br>
<bignumber>`r prettyNum(shootings_city$last12mos, big.mark=",")`<br></bignumber>
<p>Last 12 months<br>Through  `r format(lapd_asofdate, "%B %d")`</p>
</div>
<div class="card" style="background-color:#99a0a5;">
<p>Average Shootings<br>
<bignumber>`r prettyNum(round(shootings_city$avg_prior3years,0), big.mark=",")`<br></bignumber><p>
Yearly average<br>2019 to 2021</p>
</div>
<div class="card" style="background-color:#99a0a5;">
<p>Shooting Rate<br>
<bignumber>`r round(shootings_city$rate_last12,0)`<br></bignumber><p>
Per 100,000 people<br>Last 12 months</p>
</div>
<div class="card" style="background-color:#99a0a5;">
<p>Average Shooting Rate<br>
<bignumber>`r round(shootings_city$rate_prior3years,0)`<br></bignumber><p>
Per 100,000 people<br>2019 to 2021</p>
</div>
</div>
<br>
Shootings over the last 12 months are <h8>`r ifelse(shootings_city$inc_prior3yearavgtolast12>=0," up"," down")` `r ifelse(shootings_city$inc_prior3yearavgtolast12>=0,shootings_city$inc_prior3yearavgtolast12,substring(shootings_city$inc_prior3yearavgtolast12,2))`%</h8> compared to the annual average over the last three years, according to the latest Los Angeles Police Department data through `r format(lapd_asofdate, "%B %d")`.

The city averaged <h8>`r round(shootings_city$last12mos/52,0)`</h8> shootings a week over the last 12 months. In 2019, that number was <h8>`r round(shootings_city$total19/52,0)`</h8> a week.

As reported here, shootings include the number of victims shot in crime incidents recorded by the LAPD. The total may not include every shooting in the city, only those that were recorded as a crime by Los Angeles police.

<subhead>A closer look at Los Angeles shootings neighborhood by neighborhood</subhead>
...
The map color codes each police district by the shooting rate over the last 12 months. The three darker blues represent neighborhoods with robbery rates that are higher than the citywide rate. You can also click the box in the bottom right corner to see neighborhoods by the number of shootings.
...
# Set bins for numbers of crimes for shootings map
robberybins1 <- c(0,
                100,
                200,
                500,
...
robberypal1 <- colorBin(c("#aecfe6",
                             ...
                             "#0058f6",
                             "#003ca6",
                             "#00215c"), shootings_district$rate_last12, bins = robberybins1,na.color = "#51585f")

robberypal2 <- colorBin(c("#aecfe6",
                             "#2787d6",
                             "#0058f6",
                             "#003ca6",
                             "#00215c"), shootings_district$last12mos, bins = robberybins2,na.color = "#51585f")


# Create quick labels for shootings map
robberylabel <- paste(sep="",
                     "<popuptitle>",
                     shootings_district$last12mos,
                     " shootings </popuptitle><br>",
                     " in the last 12 months in the <b>",
                     shootings_district$district,
                     "</b>area, home to an estimated ",
                     prettyNum(shootings_district$population, big.mark=","),
                     " people.
<br>
<table>      
      <tr>
			...
			<tr>
				<td>2019</td>
				<td>",
shootings_district$total19,
"</td>
				<td>",
round(shootings_district$rate19,0),
"</td>
			</tr>
			<tr>
				<td>2020</td>
				<td>",
shootings_district$total20,
"</td>
				<td>",
round(shootings_district$rate20,0),
"</td>
			</tr>
						<tr>
				<td>2021</td>
				<td>",
shootings_district$total21,
"</td>
				<td>",
round(shootings_district$rate21,0),
"</td>
			</tr>
						<tr>
...
"</td>
				<td>",
round(shootings_district$rate_last12,0),
"</td>
			</tr>
						<tr>
				<td>3-year Average</td>
				<td>",
round(shootings_district$avg_prior3years,0),
"</td>
				<td>",
round(shootings_district$rate_prior3years,0),
"</td>
			</tr>
</table>")

# Creating police beats map for types of crimes
losangeles_robbery_map <- leaflet(shootings_district, options = leafletOptions(zoomControl = FALSE, zoomSnap = 0.5, zoomDelta=0.5)) %>%
...
L.control.zoom({ position: 'topright' }).addTo(this)
  ...
  addLegend(opacity = 0.6,
            values = shootings_district$rate_last12, 
            pal = robberypal1,
            position = "bottomleft", 
            title = paste(sep="","<popuptitle>Robbery Rate<br><popupchatter>Shootings/100K people<br>Citywide: ",prettyNum(round(shootings_city$rate_last12,0), big.mark=",")),
            group = "Rate",
            className = "info legend Rate") %>%
  addLegend(opacity = 0.6,
            values = shootings_district$last12mos, 
            pal = robberypal2,
            position = "bottomleft", 
            title = paste(sep="","<popuptitle>Shootings<br><popupchatter>Total shootings<br>Citywide: ",prettyNum(round(shootings_city$last12mos,0), big.mark=",")),
            group = "Number",
